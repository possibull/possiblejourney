...
extension UITestHelper {
    ...
    func navigateToSettings() throws {
        ...
        // The settings button is a NavigationLink with accessibility identifier "SettingsButton"
        let settingsButton = app.buttons["SettingsButton"]
        if settingsButton.exists {
            settingsButton.tap()
        } else {
            // Try to find settings as a NavigationLink
            let settingsLink = app.otherElements["SettingsButton"]
            if settingsLink.exists {
                settingsLink.tap()
            } else {
                // Try to find settings by the gear icon
                let gearIcon = app.images.matching(identifier: "gearshape.fill").firstMatch
                if gearIcon.exists {
                    gearIcon.tap()
                } else {
    ...
    func resetToCleanState() throws {
        // Terminate and relaunch the app with clean UserDefaults
        app.terminate()
        
        // Set launch arguments to clear UserDefaults
        app.launchArguments = ["--uitesting", "--uitesting-reset"]
        
        app.launch()
        Thread.sleep(forTimeInterval: 3) // Wait for app to fully load
    }
...
class UITestHelperMixin: XCTestCase, UITestHelper {
    ...
    override func setUpWithError() throws {
        ...
        // Clear UserDefaults and app state for clean testing
        app.launchArguments = ["--uitesting", "--uitesting-reset"]
